...
요청한 서비스가 완료되면 그제서야 다음 코드가 이어서 실행된다.
...
Parallel code 란 여러 단위가 동시에 실행되는 코드이다.

Asynchronous code 가 꼭 Parallel code 인 것은 아니다.
간단한 Asynchronous code 는 프레임웍에 서비스를 요청한 후 하염없이 대기하는 경우가 대부분이다.

하지만 Parallel code 를 만들려면 Asynchronous 환경이 조성되어 있어야 한다.
Asynchronous code 를 여럿 동시에 실행하는 것이 Parallel 이기 때문이다.

Concurrency 는 Asynchronous, Parallel 개념을 싸잡아 말할 때 쓴다.
...
Swift 6.2 부터 @MainActor 어노테이션을 붙이지 않아도 모든 타입과 펑션은 기본적으로 MainActor 에 있다고 가정한다.
이제 MainActor를 직접 표시할 필요는 거의 없어졌다.
기본 코드가 MainActor로 격리되어 있다는 개념만 알면 된다.

## @concurrent

기본 코드가 MainActor에서 실행되고 있다면 백그라운드 실행은 어떻게 시작할까.

펑션에 @concurrent 를 붙이면 된다.
...
하나가 아니라 비슷한 작업을 다발로 실행하려면 @concurrent + TaskGroup 을 사용한다.
...
과거엔 백그라운드 실행을 위해 Task.detached 를 썼는데 이제 필요없다.
@concurrent 만 쓰면 된다.

## Task

다 좋은데 Swift 코드의 기본은 Synchronous 하다. 
Asynchronous 코드를 쓰려면 Task 나 async 펑션을 만들어야 한다.

Synchronous 흐름중에 Asynchronous 코드를 쓰려면 Task로 구획을 준다.
...
주의할 점. Task 자체가 백그라운드 / 병렬처리 시작을 만들지 않는다.
...
사용자 코드는 주로 MainActor에서 실행된다. 대부분의 Task 는 MainActor를 물려받아 MainActor에서 실행된다.
...
내 모든 코드는 보통 MainActor에서 실행된다.
애플 기본 프레임웍이 제공하는 비동기 함수들만 잘 써도 왠만하면 충분하다.
이때는 Task + async/await 정도만 쓰면 된다.
Asynchronous code.
...
백그라운드에서 비슷한 작업을 다발로 돌려야 할 경우, @concurrent 에 TaskGroup 까지 붙여쓴다. Full core parallel.

Actor 경계를 넘나드는 타입이 필요한 경우.
Sendable 하게 만들어야 한다.
단순 밸류라면 자동으로 Sendable 하다.
자동으로 안 될 때는 원인에 따라 nonisolated, @unchecked Sendable 을 사용해야 한다.
